...
@pytest.mark.parametrize("model_class", [
    GraphDiTWrapper, ADiTWrapper, DMolWrapper, MolXLWrapper
])
def test_baseline_smoke(model_class):
    ...
    print(f"✅ {model_class.__name__}: validity={metrics['validity']:.3f}, runtime={runtime:.1f}s")
...
def test_all_baselines_sequential():
    ...
    for model in models:
        ...
        print(f"✅ {model.model_name}: validity={metrics['validity']:.3f}, runtime={runtime:.1f}s")
...
def test_gpu_memory_usage():
    ...
    assert memory_used < max_memory_gb, f"Memory usage {memory_used/1024**3:.1f}GB > 8GB"
