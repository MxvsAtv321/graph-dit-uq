...
def test_reward_registry():
    ...
    for reward_name in expected_rewards:
        assert (
            reward_name in REWARD_REGISTRY
        ), f"Reward {reward_name} not found in registry"
        assert callable(
            REWARD_REGISTRY[reward_name]
        ), f"Reward {reward_name} is not callable"
...
def test_sucb_reward():
    ...
    archive = [{"objectives": [0.5, -8.0, 0.6]}, {"objectives": [0.7, -6.0, 0.8]}]
    molecule = "CCO"
...
def test_reward_gradients():
    ...
    class MockUncertaintyModel:
        def predict(self, mol):
            return {"qed": 0.6, "docking": -8.0, "sa": 0.7}, {
                "qed": 0.1,
                "docking": 0.5,
                "sa": 0.1,
            }
...
def test_reward_performance():
    ...
    print(
        f"Performance: AHI={ahi_time:.3f}s, LPEF={lpef_time:.3f}s, SUCB={sucb_time:.3f}s"
    )
