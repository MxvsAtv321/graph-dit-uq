...
def run_quickvina2(
    smiles: str, receptor_path: str, exhaustiveness: int = 8
) -> Dict[str, Any]:
    """
...
async def health_check():
    ...
    return HealthResponse(
        status="healthy", timestamp=time.time(), gpu_available=gpu_available
    )
        ...
        for i in range(0, len(request.smiles), batch_size):
            ...
            for smiles in batch:
                result = run_quickvina2(
                    smiles, request.receptor_pdbqt, request.exhaustiveness
                )
...
async def root():
    ...
    return {
        ...
        "version": "1.0.0",
        "gpu_available": check_gpu(),
        "endpoints": {"health": "/live", "dock": "/dock"},
    }
