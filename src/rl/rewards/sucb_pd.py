...
def sucb_pareto_dominance_reward(
    molecule: str,
    ...
    try:
        ...
        weighted_ucb = weighted_mean + kappa * np.sqrt(
            np.dot(ucb_weights**2, sigma_vec**2)
        )
        ...
        reward = torch.nn.functional.softplus(
            torch.tensor(weighted_ucb + dominance_bonus)
        )
