    ...
    def sample_preference(self, mode: str = "adaptive") -> np.ndarray:
        ...
        if self.iteration < self.warmup_samples:
            ...
            if mode == "adaptive":
                ...
                active_objectives = np.random.choice(
                    self.n_objectives, stage + 1, replace=False
                )
...
class PreferenceSampler:
    ...
    def __init__(
        self, n_objectives: int = 3, warmup_samples: int = 100, mode: str = "adaptive"
    ):
    ...
    def update(
        self, achieved_objectives: np.ndarray, metadata: Optional[Dict[str, Any]] = None
    ):
        """
