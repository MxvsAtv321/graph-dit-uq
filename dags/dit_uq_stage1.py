...
qm9_expected_sha256 = Variable.get("QM9_SHA256", "d41d8cd98f00b204e9800998ecf8427e")  # Placeholder

# Telemetry helper with W&B session guards
def emit_metrics(task_id: str, duration_s: float, gpu_hours: float = 0.0, kg_co2: float = 0.0, run_id: str = None, **kwargs):
...
def uq_predict_fn(**context):
    ...
    if not matching_files:
        raise AirflowFailException(f"No generated molecules file found for run {run_id}")
    ...
    print(f"Added UQ predictions for {len(valid_df)} molecules, success rate: {success_rate:.2%}")
    
    # Fail if success rate too low
    if success_rate < 0.95:
        raise AirflowFailException(f"UQ prediction success rate {success_rate:.2%} below 95% threshold")
...
def validate_properties_fn(**context):
    ...
    flagged_percentage = (flagged_molecules / total_molecules * 100) if total_molecules > 0 else 0
    ...
    print(f"Validation complete: {passed_molecules}/{total_molecules} passed ({100-flagged_percentage:.1f}%)")
...
def parquet_merge_fn(**context):
    ...
    pipeline_stats = {
        ...
        'successful_docking': df['binding_affinity'].notna().sum() if 'binding_affinity' in df else 0,
    ...
    print(f"Pipeline complete: {pipeline_stats['passed_filters']}/{pipeline_stats['total_molecules']} molecules passed all filters")
...
download_data >> generative_sample >> uq_predict >> docking_score >> validate_properties >> parquet_merge 