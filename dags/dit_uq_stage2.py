...
rl_checkpoint_path = Variable.get(
    "RL_CHECKPOINT_PATH", "/data/checkpoints/graph_dit_10k.pt"
)
hard_negatives_path = Variable.get(
...
""",
    ...
    environment={"AIRFLOW_CTX_DAG_RUN_ID": "{{ run_id }}", "CUDA_VISIBLE_DEVICES": "0"},
    dag=dag,
)
...
def evaluate_results(**context):
    ...
    if training_log:
        ...
        training_metrics = {
            ...
            "reward_improvement": (
                final_rewards[-1] - final_rewards[0] if len(final_rewards) > 1 else 0
            ),
