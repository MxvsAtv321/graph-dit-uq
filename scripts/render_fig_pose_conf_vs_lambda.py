...
def render_pose_confidence_figure(csv_path, output_path):
    ...
    plt.plot(df['lambda'], df['pose_conf>0.6'] * 100, marker='s', linewidth=2, markersize=8)
    ...
    print(f"📊 Pose confidence range: {df['pose_conf>0.6'].min()*100:.1f}% - {df['pose_conf>0.6'].max()*100:.1f}%")

def main():
    parser = argparse.ArgumentParser(description='Render pose confidence vs lambda figure')
