    ...
    try:
        with open(filepath, 'r') as f:
            ...
            if 'shrirang' in content or 'shivesh' in content:
                print(f"  ⚠️  Contains author name (expected for final submission)")
                return True
            else:
                print(f"  ✅ Anonymous")
                return True
    except:
        print(f"  ⚠️  Could not check anonymity")
        return False
    ...
    try:
        with open('src/rl/molecular_ppo.py', 'r') as f:
            ...
            if 'set_seeds(42)' in content or 'torch.manual_seed(42)' in content:
                print(f"  ✅ Random seeds fixed")
                return True
            else:
                print(f"  ❌ Random seeds fixed")
                return False
    except:
        print(f"  ⚠️  Could not check random seeds")
        return False
