...
for f in glob.glob("ablation/raw/*.parquet"):
    ...
    print(f"✅ Processed {filename}: λ={lam}, Physics={mean_physics:.4f}, Conf={conf:.3f}, Molecules={len(df)}")
...
if len(records) > 0:
    ...
    if baseline and best_lambda != 0.0:
        improvement = ((best_physics['mean_physics_reward'] - baseline['mean_physics_reward']) / baseline['mean_physics_reward']) * 100
        print(f"  Improvement vs baseline (λ=0.0): {improvement:.1f}%")
        
        if improvement >= 20:
            print(f"  ✅ PASS: Physics reward improves by ≥20%")
        else:
            print(f"  ⚠️  WARNING: Physics reward improvement <20%")
    
    if best_physics['pose_conf>0.6'] >= 0.3:
        print(f"  ✅ PASS: Pose confidence ≥30%")
    else:
        print(f"  ⚠️  WARNING: Pose confidence <30%")

print(f"\n🎉 λ-SWEEP ANALYSIS COMPLETE!")
print(f"📄 Results available in: ablation/lambda_sweep_summary.csv") 