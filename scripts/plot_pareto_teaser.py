...
def plot_pareto_teaser(
    results_path: str = "outputs/results_10k.pkl",
    ...
    ax2.axvline(
        np.mean(qed), color="red", linestyle="--", label=f"Mean: {np.mean(qed):.3f}"
    )
    ...
    ax3.axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Mean: {np.mean(docking):.3f}",
    )
    ...
    # Overall title
    fig.suptitle(
        "Graph DiT-UQ: Early Results - 10k Generated Molecules",
        fontsize=20,
        weight="bold",
        y=0.98,
    )
    ...
    print(
        f"Pareto optimal: {len(pareto_indices):,} ({len(pareto_indices)/len(results)*100:.1f}%)"
    )
...
def main():
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default="screenshots/early_pareto.png",
        help="Output path for plot",
    )
    parser.add_argument(
        "--update_readme", action="store_true", help="Update README with plot section"
    )
