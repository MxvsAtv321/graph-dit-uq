...
def generate_random_smiles(n_molecules: int = 10000) -> List[str]:
    ...
    fragments = [
        ...
        "C1=CC=CC=C1", "C1CCCCC1", "C1CCCCCC1",
        "CCO", "CCCO", "CCCCCO",
        "CC(C)C", "CC(C)(C)C", "CC(C)(C)(C)C",
        "CCOC", "CCOCC", "CCOCCC",
        "CCN", "CCCN", "CCCCN",
        "CC(=O)O", "CC(=O)OC", "CC(=O)N",
        "CCOC(=O)C", "CCN(C)C", "CC(C)OC",
        "C1=CC=C(C=C1)C", "C1=CC=C(C=C1)CC",
        "C1=CC=C(C=C1)CCC", "C1=CC=C(C=C1)CCCC"
    ]
...
def compare_pareto_fronts(graph_dit_results: List[Dict], baseline_results: List[Dict]) -> Dict:
...
def main():
    ...
    parser.add_argument("--graph_dit_results", type=str, default="outputs/results_10k.pkl",
                       help="Path to Graph DiT results (default: outputs/results_10k.pkl)")
    parser.add_argument("--output_path", type=str, default="outputs/baseline_comparison.pkl",
    ...
    print("="*60)
    print(f"Graph DiT Results:")
    print(f"  Total molecules: {comparison['graph_dit_total']:,}")
    print(f"  Pareto optimal: {comparison['graph_dit_pareto']:,} ({comparison['graph_dit_pareto_ratio']*100:.2f}%)")
    ...
    print(f"  Pareto optimal: {comparison['baseline_pareto']:,} ({comparison['baseline_pareto_ratio']*100:.2f}%)")
    ...
    print(f"  QED improvement: {comparison['graph_dit_best_qed'] - comparison['baseline_best_qed']:.3f}")
    print(f"  Docking improvement: {comparison['baseline_best_docking'] - comparison['graph_dit_best_docking']:.3f}")
    print("="*60)
