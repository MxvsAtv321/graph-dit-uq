...
def aggregate_lambda_sweep_results():
    ...
    data_dir = "data"
    lambda_sweep_files = glob.glob(os.path.join(data_dir, "stage3_results_lambda_sweep_*.parquet"))
    ...
    for lambda_val in sorted(set(lambda_values)):
        ...
        if 'quickvina_score' in lambda_data.columns:
            print(f"  Mean QuickVina score: {lambda_data['quickvina_score'].mean():.4f}")
        ...
        summary_stats.append({
            ...
            'mean_physics_reward': float(lambda_data['physics_reward'].mean()) if 'physics_reward' in lambda_data.columns else None,
            'max_physics_reward': float(lambda_data['physics_reward'].max()) if 'physics_reward' in lambda_data.columns else None,
            'high_conf_pct': float((lambda_data['diffdock_confidence'] > 0.6).mean() * 100) if 'diffdock_confidence' in lambda_data.columns else None,
            'mean_quickvina': float(lambda_data['quickvina_score'].mean()) if 'quickvina_score' in lambda_data.columns else None,
            'best_quickvina': float(lambda_data['quickvina_score'].min()) if 'quickvina_score' in lambda_data.columns else None,
            'drug_like_pct': float((lambda_data['qed'] > 0.5).mean() * 100) if 'qed' in lambda_data.columns else None
        })
