...
def launch_lambda_sweep():
    ...
    for i, lambda_val in enumerate(lambda_values):
        ...
        cmd = [
            ...
            "--run-id", run_id,
            "--conf", f'{{"iters":{iterations_per_sweep},"lambda_diffdock":{lambda_val}}}'
        ]
    ...
    print(f"  # Check all runs:")
    print(f"  docker compose exec airflow-worker airflow dags list-runs --dag-id dit_uq_stage3 | grep '{run_id_prefix}'")
    print(f"  # Monitor progress:")
    print(f"  watch -n 30 'docker compose exec airflow-worker airflow dags list-runs --dag-id dit_uq_stage3 | grep \"{run_id_prefix}\" | grep success | wc -l'")
    ...
    print(f"📊 Results will be available in: ablation/raw/")
