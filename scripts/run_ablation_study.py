...
def run_ablation_study():
    ...
    experiments = [
        ...
        },
        {
            ...
            "command": [
                "python",
                "scripts/train_rl_with_uncertainty.py",
                "--n_iterations",
                "20",
                "--lambda_qed",
                "0.3",
    ...
    for exp in experiments:
        ...
        if exp["command"]:
            ...
            # Load results
            result_files = list(
                Path("outputs").glob(f"rl_results_*{exp['name']}*.json")
            )
            if result_files:
...
def create_ablation_comparison_plot(results):
    ...
    pareto_coverage = [
        results[e]["final_evaluation"]["pareto_percentage"] * 100 for e in experiments
    ]
    validity_rates = [
        results[e]["final_evaluation"]["validity_rate"] * 100 for e in experiments
    ]
    ...
    bars = ax.bar(
        range(len(experiments)), pareto_coverage, color=colors, edgecolor="black"
    )
    ...
    ax = axes[0, 1]
    bars = ax.bar(
        range(len(experiments)), validity_rates, color=colors, edgecolor="black"
    )
    ax.set_xticks(range(len(experiments)))
    ax.set_xticklabels(
        [e.replace("_", "\n") for e in experiments], rotation=45, ha="right"
    ...
    bars = ax.bar(
        range(len(experiments)), improvements, color=colors, edgecolor="black"
    )
    ax.set_xticks(range(len(experiments)))
    ax.set_xticklabels(
        [e.replace("_", "\n") for e in experiments], rotation=45, ha="right"
    ...
    # Save figure
    fig.savefig(
        "figures/publication/figure_4_ablation_study.pdf", dpi=300, bbox_inches="tight"
    )
    fig.savefig(
        "figures/publication/figure_4_ablation_study.png", dpi=300, bbox_inches="tight"
    )
