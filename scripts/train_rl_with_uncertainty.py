...
def train_rl_with_ablation(args):
    ...
    for iteration in range(args.n_iterations):
        ...
        if (iteration + 1) % args.save_frequency == 0:
            save_path = Path(f"checkpoints/rl_iter_{iteration+1}_{'with' if args.use_uncertainty else 'without'}_uncertainty.pt")
            torch.save({
                'iteration': iteration,
    ...
    results = {
        ...
        'metrics': {
            ...
            'pareto_coverage': [float(p) for p in metrics['pareto_coverage']],  # Convert numpy to float
            'valid_molecules': metrics['valid_molecules'],
            'unique_molecules': list(metrics['unique_molecules']),  # Convert set to list
...
def evaluate_rl_progress(ppo, n_molecules=1000):
    ...
    for i in range(n_molecules):
        ...
        ppo.generator.eval()
        with torch.no_grad():
            smiles = ppo.generator.decode_action(torch.tensor([i % 10]))  # Mock generation
    ...
    return {
        ...
        'mean_qed': np.mean([m['qed'] for m in valid_molecules]) if valid_molecules else 0,
        'mean_docking': np.mean([m['docking_score'] for m in valid_molecules]) if valid_molecules else 0,
        'mean_sa': np.mean([m['sa_score'] for m in valid_molecules]) if valid_molecules else 0,
    ...
    for i, molecule in enumerate(molecules):
        ...
        for j, other in enumerate(molecules):
            if i != j:
                ...
                if (other['qed'] >= molecule['qed'] and 
                    other['docking_score'] <= molecule['docking_score'] and
                    other['sa_score'] <= molecule['sa_score']):
                    if (other['qed'] > molecule['qed'] or 
                        other['docking_score'] < molecule['docking_score'] or
                        other['sa_score'] < molecule['sa_score']):
                        dominated = True
                        break
        if not dominated:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--checkpoint_path', type=str, 
                       default='checkpoints/graph_dit_10k.pt')
