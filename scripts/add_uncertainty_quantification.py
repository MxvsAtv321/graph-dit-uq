        ...
        enhanced_result.update(
            {
                ...
                "uncertainty_score": np.mean(
                    [qed_variance, docking_variance, sa_variance, logp_variance]
                ),
            }
        )
...
def update_pareto_plot_with_uncertainty(results_path: str, output_path: str):
    ...
    # Overall title
    fig.suptitle(
        "Graph DiT-UQ: Uncertainty-Aware Multi-Objective Optimization",
        fontsize=18,
        weight="bold",
        y=0.98,
    )
...
def analyze_uncertainty_correlation(results: List[Dict]) -> Dict:
    ...
    high_uncertainty = [
        r for r in results if r["uncertainty_score"] > uncertainty_threshold
    ]
    low_uncertainty = [
        r for r in results if r["uncertainty_score"] <= uncertainty_threshold
    ]
    ...
    analysis = {
        ...
        "high_uncertainty_avg_docking": np.mean(
            [r["docking"] for r in high_uncertainty]
        ),
...
def main():
    ...
    parser.add_argument(
        "--output_path",
        type=str,
        default="screenshots/uncertainty_pareto.png",
    ...
    enhanced_results = update_pareto_plot_with_uncertainty(
    ...
    print(
        f"Docking-Uncertainty Correlation: {analysis['docking_uncertainty_correlation']:.3f}"
    )
    print(
        f"Uncertainty Range: {analysis['uncertainty_range'][0]:.4f} - {analysis['uncertainty_range'][1]:.4f}"
    )
    print(
        f"Uncertainty Mean ± Std: {analysis['uncertainty_mean']:.4f} ± {analysis['uncertainty_std']:.4f}"
    )
    ...
    print(
        f"📁 Enhanced results: {args.results_path.replace('.pkl', '_with_uncertainty.pkl')}"
    )
